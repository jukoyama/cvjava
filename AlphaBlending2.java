...
public class AlphaBlending2 {
	...
	static MyImage execute(MyImage input1, MyImage input2, MyImage input0, int a) {
		...
		MyImage output = new MyImage(width, height);
			...
			int y = (height - height1) * a;

		for(int i = 0; i < height; i++) {
			for(int j = 0; j < width; j++) {
				boolean isProcessed = false;

				if(i < height1 + y && j < width1+ x) {

					if(i >= y && i <= y + height1 && j >= x && j <= x + width1) {

						Color color0 = input0.getColor(j - x, i - y);
						if(color0.getRed() > 0 || color0.getBlue() > 0 || color0.getGreen() > 0) {

							Color color1 = input1.getColor(j - x, i - y);
							output.setColor(j, i, color1);
							//output.setColor(j, i, Color.red);
